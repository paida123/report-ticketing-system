      ...
      <div className="table-wrap um-table-wrap">
        <table className="user-tickets-table">
          <thead>
            <tr>
              <th>Role</th>
              <th className="subject-col">Description / Permissions</th>
          ...
          <tbody>
            {roles.map(r => (
              <tr key={r.id}>
                ...
                <td className="actions-col">
                  <button className="icon-btn" onClick={() => { setEditing(r); setEditOpen(true); }} title="Edit" aria-label={`Edit role ${r.name}`}>
            ...
            {roles.length === 0 && (
              <tr><td colSpan={3} className="muted" style={{ textAlign: 'center' }}>No roles.</td></tr>
            )}
          </tbody>
        </table>
      </div>
