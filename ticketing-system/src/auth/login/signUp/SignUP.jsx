...
const SignUp = () => {
  ...
  useEffect(() => {
    if (step === 1) {
      ...
      setErrors(validateStep2());
    }
    /* eslint-disable-next-line */
  }, [email, tempPassword, newPassword, confirmPassword, step]);

  const handleVerifyTempPassword = async (ev) => {
    ev.preventDefault();
    setTouched({ email: true, tempPassword: true });
    ...
    setServerMessage('');
    try {
      const response = await AuthenticationService.verifyTempPassword(email, tempPassword);
      const userData = response.data?.data;
      setUserName(userData?.first_name || '');
      setServerMessage(response.data?.message || 'Temporary password verified successfully!');
      setTimeout(() => {
        setStep(2);
        setServerMessage('');
      }, 1000);
    } catch (err) {
      const errorMsg = err.response?.data?.message || 'Invalid temporary password. Please try again.';
  ...
  const handleSetPassword = async (ev) => {
    ev.preventDefault();
    ...
    setServerMessage('');
    try {
      const response = await AuthenticationService.setupPassword(email, tempPassword, newPassword);
      setServerMessage(response.data?.message || 'Password set successfully. You may now login.');
      ...
      <div className="left-side">
        <div className="left-content">
          ...
          <div className="brand-text">
            <h2 className="brand-title">Join Our Platform</h2>
            <p className="brand-subtitle">Set up your account and start managing tickets efficiently.</p>
          </div>
        </div>
      </div>
          ...
          <div className="login-card">
            <div className="card-header">
              <h1 className="welcome-title">
                {step === 1 ? 'Verify Your Account' : `Welcome, ${userName}!`}
              </h1>
            ...
            {serverMessage && (
              <div className={`server-info ${serverMessage.includes('successfully') || serverMessage.includes('verified') ? 'success' : ''}`} role="status">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  {serverMessage.includes('successfully') || serverMessage.includes('verified') ? (
                    <>
            ...
            {step === 1 ? (
              <form onSubmit={handleVerifyTempPassword} noValidate className="login-form">
                {/* Email */}
                <div className="form-group">
                  <label htmlFor="email">Email</label>
                  <div className={`input-wrapper ${emailFocused ? 'focused' : ''} ${touched.email && errors.email ? 'error' : ''}`}>
                    <div className="input-icon">
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M3 5h14a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2z"></path>
                        <path d="m3 7 7 5 7-5"></path>
                      </svg>
                    </div>
                    <input
                      id="email"
                ...
                {/* Verify Button */}
                <button type="submit" className="login-button" disabled={verifying || Object.keys(errors).length > 0}>
                  {verifying ? (
                    <>
              ...
              <form onSubmit={handleSetPassword} noValidate className="login-form">
                {/* New Password */}
                <div className="form-group">
